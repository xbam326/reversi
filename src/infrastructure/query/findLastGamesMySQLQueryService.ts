...
import { FindLastGamesQueryModel, FindLastGamesQueryService } from "../../application/query/findLastGameQueryService";
  ...
  async query(conn: mysql.Connection, limit: number): Promise<FindLastGamesQueryModel[]> {
    const selectResult = await conn.execute<mysql.RowDataPacket[]>(
      ...
      select
        ...
        sum(case when m.disc = 1 then 1 else 0 end) as light_move_count,
